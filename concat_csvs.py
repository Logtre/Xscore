...
class ConcatCsvsAll:
    ...
    def concat(self, out_path, out_path2):
        ...
        '''
        # product_attr
        feat_product_attr = self.count_rec.group_sum(self.average_attr, index_col='顧客ID', aggregate_col=['売上単価','売上','数量'])
        feat_product_attr.columns = ['売上単価','売上','数量']
        print(feat_product_attr.head())
        feat_product_attr = pd.merge(self.id, feat_product_attr, on='顧客ID',how='left')
        #feat_product_attr = feat_product_attr.drop('Unnamed: 0', axis=1)
        # product_attr
